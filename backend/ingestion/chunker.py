...
DOCUMENT_CONFIG = {
    # These IDs/titles mirror the notebook configuration.
    "WHO-BC-2023-001": {
        "source": "WHO",
...
def split_into_sections(page_text: str) -> list[dict[str, str]]:
...
def _document_config(document_id: str) -> dict:
    if document_id not in DOCUMENT_CONFIG:
        raise KeyError(
            f"No document configuration for {document_id}"
        )
    return DOCUMENT_CONFIG[document_id]
...
def chunk_documents(
    ...
    for page in pages:
        ...
        if not document_id:
            raise ValueError(
                "Every page must contain document_id before chunking."
            )

        config = _document_config(document_id)
        ...
        for section in sections:
            ...
            for chunk in section_chunks:
                ...
                chunk.metadata = build_document_metadata(
